...
(function ($)
{
	...
	$.ClosePumaSideBar = function(Animate){
	...
	$.PumaSideBar = function (settings,callback)
    {
    	psbOpen = 0;
		...
		if(settings.position == "left"){
 		...
 		TempTimer = setTimeout(function() {
 			$("#PumaSideBar").attr("mouseover","0");
 		}, 300);

 		$("#PumaSideBar").attr("mouseover");
...
// ================== Creates the first lvl of items
    	...
    	function Loadlvl1(){
    		...
    		for(i=0; i< glbSettings.items.length; i++){
    			...
    			var css_link = '';
    			if(childs<1)css_link = 'w100';
    			//if(link !=undefined)

    			link = (link)?'href="'+ link +'" ':'';
    			psbOption += '<a class="psbLink '+css_link+'"  '+link + target + ">";
        ...
        function AjustMenu(){
        	...
        	$(".psbSwiper-ContainerMain").css("height", SpaceNeeded+"px");
        	if(SpaceNeeded <0)
        		alert(SpaceNeeded);
        }

			$(document).on("mouseenter","#PumaSideBar",function(){
        		$(this).attr("mouseover","1");
        	});

        	$(document).on("mouseover","#PumaSideBar",function(){
        		$(this).attr("mouseover","1");
        	});

        	$(document).on("mouseleave","#PumaSideBar",function(){
        	...
        	$(document).on("click",function(){
        		...
        		if(glbSettings.closeoutside){
	        		...
	        		var MouseOver = $("#PumaSideBar").attr("mouseover");
	        		...
	        		if(MouseOver != 1){
	        			...
	        			if(glbCallback!=undefined){
		        			...
		        			if (typeof glbCallback == "function" && $("#PumaSideBar").length>0 )
			                {
			    ...
			    if(glbSettings.closeoutside){
				    ...
				    if(glbSettings.position=="left"){
					    ...
					    if(touch.pageX>210){
					    	$.ClosePumaSideBar(1);
					    	if(glbCallback!=undefined)
					    	if (typeof callback == "function" && $("#PumaSideBar").length>0 )
			                {
			                    if(glbCallback)glbCallback(undefined,"Closing");
			                }
					    }
		    ...
		    $(document).on("click",".psbOption",function(){
				...
				var subarray    = $(this).attr("subarray")
				var subsubarray = $(this).attr("subsubarray")
				...
				if(childs>0){
					...
					$("#psbSubIcon").removeClass().addClass("fa fa-5x "+ fa);
