      ...
      <div class="wrapper">
        <ul class="menu">
          ...
          <li>
            ...
            <a href="{{ $items->relative_url.$items->url }}/1">{{ $items->name }}</a>
            @else
            <a href="{{ SITE_URL.('item/'.$items->url) }}/1">{{ $items->name }}</a>
            @endif
            <ul class="sub-menu">
                    @foreach ($items->childs as $child)
                    @if($child->relative_url)
                    <li><a href="{{ $child->relative_url.$child->url }}/1">{{ $child->name }}</a></li>
                    @else
                    <li><a href="{{ SITE_URL.('item/'.$child->url) }}/1">{{ $child->name }}</a></li>
                    @endif
                    @endforeach
            </ul>
          </li>
